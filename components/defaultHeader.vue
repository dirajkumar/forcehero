<template>
  <div>
    <v-toolbar :color="this.$vuetify.theme.primary" dark app>
      <v-toolbar-side-icon class="hidden-lg-and-up" @click="onDrawerClick" />
      <v-toolbar-title>
        <logo />
      </v-toolbar-title>
      <v-spacer />
      ...
      <v-toolbar-items class="pr-10 hidden-md-and-down">
        <template v-for="item in navigationItems">
          <v-btn v-if="!item.hasChildren" :key="item.to" :to="item.to" flat>
            {{ item.label }}
          </v-btn>
          <v-menu
